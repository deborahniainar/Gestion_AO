...
const PriceEQU = () => {
  ...
  // Dropdown refs & state for button-based selectors (match PriceMO)
                    ...
                    {lotMenuOpen && (
                      <ul style={{ zIndex: 9999 }} className="absolute right-0 mt-1 w-40 max-h-52 overflow-auto bg-white border rounded shadow-lg">
                        ...
                        {(savedLots || []).map(s => (<li key={s.id} className="px-3 py-2 hover:bg-gray-100 cursor-pointer" onClick={async () => {
                          if (lot) { saveRowsForLot(lot, rows).catch(() => { }); setRowsByLot(prev => ({ ...prev, [lot]: rows })) }
                          ...
                          const personnelsFromLot = await fetchPersonnelsForLot(lotId)
                          if (Array.isArray(personnelsFromLot) && personnelsFromLot.length > 0) setPersonnels(personnelsFromLot)
                          else setPersonnels(defaultPersonnels)
                          setLotMenuOpen(false)
                        }}>{s.name}</li>))}
                      </ul>
                    )}
